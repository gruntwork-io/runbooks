...
func (e *TestExecutor) runAssertion(assertion TestAssertion) AssertionResult {
	...
	case AssertionFilesGenerated:
		result = e.assertFilesGenerated(assertion.Block, assertion.MinCount)
...
// resolvePath resolves a path relative to the output directory.
func (e *TestExecutor) resolvePath(path string) string {
	if filepath.IsAbs(path) {
		return path
	}
	return filepath.Join(e.outputPath, path)
}
...
// assertFilesGenerated checks that a template block generated at least minCount files.
func (e *TestExecutor) assertFilesGenerated(blockID string, minCount int) AssertionResult {
	// For now, count files in the output directory
	// In the future, we could track files generated by specific template blocks
	count := 0
	filepath.Walk(e.outputPath, func(path string, info os.FileInfo, err error) error {
		if err != nil {
			return nil
		}
		if !info.IsDir() {
