...
A profile's credentials are validated in the partition of the profile's own `region`. A profile that sets no region uses the region selected in the block instead, so pick a `us-gov-*` region for a GovCloud profile without one.
