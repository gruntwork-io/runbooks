...
- `detectCredentials` (`false` or `CredentialSource[]`) - Configure automatic credential detection (default: `['env']`). See [Credential Detection](#credential-detection).
...
By default, AwsAuth automatically detects existing credentials from environment variables. When credentials are detected, the user is shown the AWS account ID and identity, and must confirm before the credentials are used.

<Aside type="note">
Unlike GitHubAuth which auto-authenticates silently, AwsAuth requires explicit user confirmation. This prevents accidental operations against the wrong AWS account, which could have serious consequences in production environments.
</Aside>

<Aside type="caution">
Detection is **read-only** - it checks your environment variables and validates them with AWS, but does NOT register them to the session. Credentials only become available to scripts after you explicitly confirm. See [Credential Protection](#credential-protection) for details.
</Aside>
...
Check for prefixed environment variables (e.g., `PROD_AWS_ACCESS_KEY_ID`):
...
- Only allows reading `PREFIX_AWS_ACCESS_KEY_ID`, `PREFIX_AWS_SECRET_ACCESS_KEY`, `PREFIX_AWS_SESSION_TOKEN`, `PREFIX_AWS_REGION`
...
### What Happens When User Rejects Detected Credentials
...
2. Any subsequent commands will NOT have AWS credentials until the user authenticates manually

This ensures that if a user sees credentials for the wrong account (e.g., production when they expected development), those credentials cannot accidentally be used.

### Detection Props Reference

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `detectCredentials` | `false` or `CredentialSource[]` | `['env']` | Sources to check for existing credentials. Set to `false` to disable. |
