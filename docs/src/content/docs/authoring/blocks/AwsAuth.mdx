...
You can include multiple AwsAuth blocks in a single gruntbook to authenticate to different AWS accounts:
...
```bash
#!/bin/bash
CREDS=$(aws sts assume-role --role-arn "$ROLE_ARN" --role-session-name gruntbook)
...
echo "AWS_REGION=us-west-2" >> "$GRUNTBOOK_OUTPUT"  # Optional: override region
```
...
Note that this behavior only applies to the AwsAuth block. If standard AWS environment variables are available and the gruntbook does not contain any AwsAuth blocks, environment credentials work normally and these AWS credentials will automatically be available to all scripts.
...
When a gruntbook contains any `<AwsAuth>` blocks, Gruntbooks automatically **strips AWS credentials from the session** at startup. This means:

1. **Before confirmation**: Even if you have `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_SESSION_TOKEN` set in your terminal, scripts in the gruntbook cannot access them until you explicitly confirm which account to use
...
This design prevents a common and dangerous scenario: you have production credentials set in your terminal, open a gruntbook intending to work in development, and accidentally run scripts against production.

<Aside type="tip">
If your gruntbook does NOT contain any AwsAuth blocks, environment credentials work normally - they're available to all scripts. The protection only activates when AwsAuth is present, since that signals the gruntbook author wants explicit credential management.
</Aside>
...
- **Credentials stay local**: All authentication happens between your machine and AWS. Credentials are never transmitted to external servers or to Gruntwork (unless your gruntbook includes scripts that explicitly do this).
- **Session storage**: Credentials are stored only in your local Gruntbooks session and are never persisted to disk.
