...
| `defaultRegion` | string | "us-east-1" | Default AWS region for CLI commands. Sets `AWS_REGION` environment variable. GovCloud regions (`us-gov-east-1`, `us-gov-west-1`) are supported |
...
| `ssoRegion` | string | "us-east-1" | AWS region where IAM Identity Center is configured. For a GovCloud Identity Center this is `us-gov-east-1` or `us-gov-west-1` |
