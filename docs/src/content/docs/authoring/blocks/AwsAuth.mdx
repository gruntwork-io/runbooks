...
| `{ env: { testPrefix: 'CI_' } }` | In tests only, check `CI_AWS_ACCESS_KEY_ID`, etc. |
| `{ env: { prefix: 'PROD_', testPrefix: 'CI_' } }` | Runtime uses `PROD_`, tests use `CI_` |
...
Use `testPrefix` to specify a different prefix for automated tests, while keeping a different prefix (or standard vars) for the end-user-facing runtime:
...
- **Runtime**: Checks standard `AWS_ACCESS_KEY_ID`, etc.
- **Tests**: Checks `CI_AWS_ACCESS_KEY_ID`, etc.

You can also combine both prefixes:
...
This is useful when you want to test with CI credentials while the interactive runbook uses production-prefixed credentials.
