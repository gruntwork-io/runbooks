...
Commands can save files to the generated files directory using the `$RUNBOOK_FILES` environment variable. Any files written to this directory will automatically appear in the file panel after the command completes successfully.
...
tofu output -json > "$RUNBOOK_FILES/tf-outputs.json"

# Copy a config file
cp config.yaml "$RUNBOOK_FILES/"
```
...
You can create subdirectories within `$RUNBOOK_FILES` to organize your captured files:

```bash
#!/bin/bash
mkdir -p "$RUNBOOK_FILES/terraform"
mkdir -p "$RUNBOOK_FILES/config"

tofu output -json > "$RUNBOOK_FILES/opentofu/outputs.json"
...
2. The `$RUNBOOK_FILES` environment variable points to this directory
3. Your script writes files to `$RUNBOOK_FILES`
...
Check out the [demo-runbook-outputs](https://github.com/gruntwork-io/runbooks/tree/main/testdata/demo-runbook-outputs) example for a complete working demonstration of block outputs, including Command blocks that produce outputs consumed by downstream Commands, Checks, Templates, and TemplateInline blocks.
</Aside>
