...
When running scripts locally (outside the Gruntbooks UI), the logging functions aren't automatically injected. To simulate the Gruntbooks environment, run this one-liner in your terminal session (bash or zsh):

```bash
mkdir -p ~/.config/gruntbooks && curl -fsSL https://raw.githubusercontent.com/gruntwork-io/runbooks/main/scripts/logging.sh -o ~/.config/gruntbooks/logging.sh && export BASH_ENV=~/.config/gruntbooks/logging.sh
```
...
```fish
mkdir -p ~/.config/gruntbooks; and curl -fsSL https://raw.githubusercontent.com/gruntwork-io/runbooks/main/scripts/logging.sh -o ~/.config/gruntbooks/logging.sh; and set -gx BASH_ENV ~/.config/gruntbooks/logging.sh
```

To make this persistent across sessions, add the following to a fish startup file (e.g., `~/.config/fish/conf.d/gruntbooks.fish`):

```fish
if test -f ~/.config/gruntbooks/logging.sh
    set -gx BASH_ENV ~/.config/gruntbooks/logging.sh
end
```
:::
...
Commands can produce **outputs** that downstream blocks consume. Scripts write `key=value` pairs to the `$GRUNTBOOK_OUTPUT` file, and downstream blocks reference them via `{{ .outputs.<block-id>.<key> }}`.
...
- **Installing tools**: Install tools needed to execute the gruntbook
