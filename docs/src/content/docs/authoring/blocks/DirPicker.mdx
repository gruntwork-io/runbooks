...
The selected path is registered as a block output, making it available to downstream `<TemplateInline>` blocks via the `{{ .outputs.<id>.PATH }}` syntax.
...
| Output | Description |
|--------|-------------|
| `PATH` | The composed directory path (relative to the workspace root). Updated on every dropdown selection or manual edit. When the path is empty (the first dropdown is reset to its placeholder or the text input is cleared), `PATH` is removed, so blocks that reference it wait until a path is chosen again. |
...
<TemplateInline
  ...
  outputPath="{{ .outputs.target_path.PATH }}/terragrunt.hcl"
  generateFile={true}
  target="worktree"
>
```
...
2. It lists the immediate subdirectories of the root directory to fill the first dropdown.
...
5. On every change (dropdown or manual edit), the block registers `PATH` as an output. Clearing the path removes the output.
