...
The `<DirPicker>` block provides a cascading set of dropdowns for picking a path within a cloned repository. 
...
Authors specify an ordered array of labels to assign to each cascading directory level. For example, `['Account', 'Region']` will create a two-level dropdown with labels "Account" and "Region". DirPicker then scans the directory structure and lets users compose a target path interactively by selecting the desired folder from each level.

Users can also type a path directly into an auto-populated and editable text input below the dropdowns.
