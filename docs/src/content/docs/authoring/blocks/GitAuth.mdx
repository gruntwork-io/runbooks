...
2. The provider CLI — `gh auth token` (GitHub) or `glab auth token` (GitLab). For GitLab, Runbooks also reads glab's `config.yml` directly (where `glab auth login` stores the token) when the `glab` binary is not on `PATH`.
