...
|--------|---------|-------|
| HTTPS | `https://github.com/org/repo.git` | Recommended. Token auth injected automatically for URLs on the authenticated GitHub host. |
...
1. **`githubAuthId`** — If specified, uses the `GITHUB_TOKEN` from the referenced GitHubAuth block's outputs, for the host that block authenticated to.
2. **Session environment** — Checks the session environment for a GitHub token bound to the host (see [environment variables and hosts](/authoring/blocks/githubauth/#environment-variables-and-hosts)).
3. **No token** — The "Browse GitHub repositories" section is hidden. The user can still clone public repos or use SSH URLs.

When a GitHub token is available and the user enters an HTTPS URL on the host that token belongs to, the token is automatically injected on the server side for authentication. The token is never exposed in the browser.
