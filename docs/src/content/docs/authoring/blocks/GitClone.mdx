...
**Cancel** stops the running `git` process, and a cancelled clone produces no outputs. `git clone` removes its partially cloned directory when stopped. A [sparse checkout](#sparse-checkout) is the exception: it runs several `git` commands, and one cancelled after the first of them (the clone itself) leaves the directory in place. Cloning to the same path again then offers **Delete & Clone** to replace it.
