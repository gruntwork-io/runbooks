...
1. **GitHub browser** — When a GitHub token is available and a repo is selected in the browser, a "Ref" dropdown appears listing all branches. The default branch is automatically pre-selected and marked with a "default" badge. For repos with many branches, type in the search field to filter.

2. **Text input** — The "Ref" field below the GitHub browser accepts any branch or tag name directly, even without GitHub authentication. Use the `prefilledRef` prop to set this in advance.
