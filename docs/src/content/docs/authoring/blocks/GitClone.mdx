...
**Cancel** stops the running `git clone` process, which removes the partially cloned directory. A cancelled clone produces no outputs.

**Clone again** (or **Choose a different repo** for a local checkout) withdraws the block's outputs and removes the repository from the workspace until the next clone or checkout completes. Later blocks wait for it rather than carrying on against the previous repository, and a repository with no commits chosen next is held back as described in [Repositories With No Commits](#repositories-with-no-commits).
