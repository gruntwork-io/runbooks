...
| `{ env: { testPrefix: 'CI_' } }` | In tests only, check `CI_GITHUB_TOKEN`, `CI_GH_TOKEN` |
...
Use `testPrefix` to specify a different prefix for automated tests:

```mdx
<GitHubAuth
  id="gh-auth"
  detectCredentials={[{ env: { testPrefix: 'CI_' } }, 'cli']}
/>
```

- **Runtime**: Checks `GITHUB_TOKEN`, `GH_TOKEN`, and CLI (default behavior)
- **Tests**: Checks `CI_GITHUB_TOKEN`, `CI_GH_TOKEN`

By default (without `testPrefix`), the test framework checks `RUNBOOKS_GITHUB_TOKEN` first, which helps avoid accidentally using credentials from your local environment when running tests.
