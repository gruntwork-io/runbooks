...
The GitHubAuth block works with `github.com`, including GitHub Enterprise Cloud organizations hosted there. GitHub Enterprise Server is not supported: tokens are always validated against `github.com`. Contact Gruntwork for enterprise support options.
...
If the referenced block has not run yet, GitHubAuth shows *Waiting for "fetch-token" to run…* and pauses detection there rather than skipping to a later source — the order you write the sources is the priority order. If the block ran without writing a usable `GITHUB_TOKEN` (for example, the script failed), detection moves on to the next source, and the manual sign-in form appears if none of them finds a credential.
...
2. You open github.com/login/device and enter the code. GitHub keeps the code valid for 15 minutes by default, and Runbooks keeps checking for your approval until it expires. If it expires before you approve, start the sign-in again.
