...
You can still authenticate to GitHub without this block by using Gruntbooks' support for environment variables, however this is the recommended way to authenticate to GitHub in Gruntbooks.
...
By default, Gruntbooks will automatically detect and use credentials from:
...
<Aside type="tip">
Using `githubAuthId` is recommended even when you only have one GitHubAuth block. It prevents users from accidentally running commands before authenticating, makes the dependency explicit in your gruntbook, and helps with automated testing.
</Aside>
...
To use a different prefix for automated tests (e.g., to avoid conflicts with local credentials), configure `env_prefix` in your test config file (`gruntbook_test.yml`) rather than in the MDX:
...
Without `env_prefix`, the test framework checks `GRUNTBOOKS_GITHUB_TOKEN` first, which helps avoid accidentally using credentials from your local environment when running tests. See [Testing GitHubAuth Blocks](/authoring/testing/#testing-githubauth-blocks) for details.
...
| **GitHub CLI** | Permissions are inherited from `gh auth login`; Gruntbooks warns if `repo` scope is missing |
| **OAuth** | Gruntbook author specifies OAuth scopes via `oauthScopes` prop; user approves on GitHub |
...
The default is `["repo"]`, which grants full read/write access to repositories. Consider requesting fewer permissions if your gruntbook has limited needs:
...
<Aside type="caution">
OAuth permissions are coarse-grained, and the minimum viable OAuth scope that Gruntbooks can use is `repo`, which grants full read/write access to all repositories and is still quite permissive. If you want to narrow down the permissions, consider defining a fine-grained token and using it as an environment variable, or manually entering it into the UI.
</Aside>

### Personal Access Tokens

When users authenticate with a PAT, the token's permissions are determined by how the user created it on GitHub. Gruntbook authors cannot control PAT permissions.
...
If your gruntbook requires specific permissions, document them clearly so users know what to configure when creating their PAT.
...
<Aside type="note">
When detecting credentials from GitHub CLI, Gruntbooks checks if the token has the `repo` scope and displays a warning if it's missing, since many common operations require it.
</Aside>
...
2. **Credentials stay in memory.** Tokens are stored only in your local Gruntbooks session and are never persisted to disk.
...
| Method | How It Works |
|--------|-------------|
| **Environment Variable** | Gruntbooks reads `GITHUB_TOKEN` from your local environment. The token never leaves your machine. |
| **GitHub CLI** | Gruntbooks calls `gh auth token` locally. The token is retrieved from your CLI's secure storage and stays local. |
| **OAuth** | Uses GitHub's [Device Authorization Grant](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps#device-flow). GitHub sends the token directly to your local Gruntbooks instance—Gruntwork's servers are never involved. |
| **Personal Access Token** | You paste the token into the local UI. It's sent directly to your local Gruntbooks server (localhost), never to any external server. |

### OAuth Flow Details

When you authenticate via OAuth:

1. Gruntbooks (running on your machine) requests a device code from GitHub
2. You open github.com/login/device and enter the code
3. You see "Gruntwork Gruntbooks wants to access your account" and approve
4. GitHub sends the token directly to your local Gruntbooks instance
...
#### Malicious Gruntbook with Custom `oauthClientId`

**Scenario**: An attacker publishes a gruntbook with `oauthClientId="attacker-app-id"`.

**What happens**:
- You see "Attacker's App" (not "Gruntwork Gruntbooks") on GitHub's authorization page
...
**Mitigation**: Despite no risk, this is still a potential indicator of suspicious activity, so Gruntbooks displays a warning when `oauthClientId` differs from the default. Always verify the app name on GitHub's authorization page before approving.

#### Malicious Gruntbook with Custom Base URL

**Scenario**: An attacker publishes a gruntbook that attempts to redirect OAuth to a fake GitHub login page (e.g., `oauthBaseUrl="https://github-login.attacker.com"`).

**What happens**: Nothing. Gruntbooks does not allow `oauthBaseUrl` as a prop. GitHubAuth only communicates with Github.com, so the attack fails before it starts.

**Risk**: None. This attack vector is blocked by design.

#### Scope Escalation

**Scenario**: A gruntbook requests excessive permissions with `oauthScopes={["repo", "admin:org", "delete_repo"]}`.

**What happens**: Gruntbooks displays the requested scopes in the UI before you authenticate. GitHub's authorization page also displays all requested scopes. If you approve without reviewing either, you grant broader access than needed.

**Risk**: Low. Both Gruntbooks and GitHub clearly display requested scopes, giving you two opportunities to review before granting access.

**Mitigation**: Review the scopes shown in Gruntbooks before clicking "Sign in with GitHub", and verify them again on GitHub's authorization page. Be suspicious of gruntbooks requesting `admin:*`, `delete_repo`, or `write:*` scopes beyond the default `repo`.
...
- You want to ensure the token in your local Gruntbooks session stops working
- You're done with a gruntbook and want to clean up
...
Alternatively, simply closing your Gruntbooks session discards any tokens stored locally.
