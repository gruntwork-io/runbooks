...
| `{ env: { prefix: 'PREFIX_' } }` | Check prefixed env vars (`PREFIX_GITHUB_TOKEN`, `PREFIX_GH_TOKEN`, …), bound to a host by `PREFIX_GH_HOST` |
...
- Only allows reading `PREFIX_GITHUB_TOKEN` / `PREFIX_GH_TOKEN` (or `PREFIX_GH_ENTERPRISE_TOKEN` / `PREFIX_GITHUB_ENTERPRISE_TOKEN` for a GitHub Enterprise Server host). `PREFIX_GH_HOST` (not the unprefixed `GH_HOST`) decides which host they are bound to, by the same rules as for the unprefixed variables
