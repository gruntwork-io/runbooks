...
The `<GitHubPullRequest>` block provides a streamlined way to create GitHub pull requests directly from a gruntbook. It integrates with the [`<GitClone>`](/authoring/blocks/gitclone/) block to push changes made during the gruntbook workflow and open a PR.
...
<GitHubPullRequest
  ...
  description="Create a PR with the changes made in this gruntbook"
/>
```
...
| `prefilledBranchName` | `string` | No | Pre-fills the branch name. Supports template expressions. Defaults to `gruntbook/<timestamp>`. |
