...
The `<GitLabMergeRequest>` block creates a GitLab merge request from changes made during a runbook. It is a GitLab-locked alias of [`<GitPullRequest>`](/authoring/blocks/gitpullrequest/) — equivalent to `<GitPullRequest provider="gitlab" />` — and accepts the same props, including `prefilledCommitMessage`.
