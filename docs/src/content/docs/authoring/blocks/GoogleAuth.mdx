...
All three tabs also offer an optional **Default Region** picker, which seeds the region environment variables for subsequent commands. It starts on `defaultRegion`; choosing **No default region** sets no region at all (on the gcloud Config tab, the configuration's own `compute/region` then applies).
...
| `CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE` | The same path, in the property the `gcloud` CLI itself reads — see [Bridging to the gcloud CLI](#bridging-to-the-gcloud-cli) |
...
| Variable | Source |
|----------|--------|
| `GOOGLE_CLOUD_REGION`, `CLOUDSDK_COMPUTE_REGION`, `GOOGLE_REGION` | `defaultRegion`, the Default Region picker, or else the gcloud configuration's `compute/region` (or, for a credential detected in environment variables, their region) |
| `CLOUDSDK_COMPUTE_ZONE`, `GOOGLE_ZONE` | `defaultZone`, or else the gcloud configuration's `compute/zone` (or the detected environment's zone) |
...
```
ERROR: (gcloud.organizations.list) UNAUTHENTICATED: Request had invalid authentication
credentials. Expected OAuth 2 access token, login cookie or other valid authentication
credential.
```

GoogleAuth closes this gap for you: whenever it materializes or points at a credentials file, it also sets `CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE` to that same path, which routes the CLI through its normal refreshable-credential code path instead of its own login state. Re-authenticating (including switching credential types) keeps this in sync automatically — you never need to set it yourself.

<Aside type="caution">
This bridging only works when the credential has a file — the [access token case](#access-token-credentials) does not, since there is nothing on disk to point the CLI at. In that one case, bare `gcloud` commands still resolve against whatever CLI login is already configured on the machine. Do not work around this by exporting a static bearer token as `CLOUDSDK_AUTH_ACCESS_TOKEN` yourself: some older Google APIs (for example Cloud Resource Manager v1's `Organizations.SearchOrganizations`, which backs `gcloud organizations list`) reject a bearer presented that way with `ACCESS_TOKEN_TYPE_UNSUPPORTED`, freshly-minted or not.
</Aside>
...
| `CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE` | The same path, in the property the `gcloud` CLI reads — blank for an [access-token-only](#access-token-credentials) credential |
...
| `GOOGLE_CLOUD_REGION`, `CLOUDSDK_COMPUTE_REGION`, `GOOGLE_REGION` | The region the block wrote to the session (see [above](#environment-variables)), if any |
| `CLOUDSDK_COMPUTE_ZONE`, `GOOGLE_ZONE` | The zone the block wrote to the session, if any |
...
The [environment variables above](#environment-variables) are automatically available to all subsequent Command and Check blocks, so blocks use the most recently authenticated GoogleAuth block without any extra configuration. Any subsequent Google Cloud authentication updates those variables and becomes the new default, and so does picking a project with **Change project** on a block's success card.
...
<Aside type="caution">
With multiple GoogleAuth blocks the session environment variables reflect the block that **most recently** authenticated or changed project. Use `googleAuthId` on every command so each one runs against the project you intended. Each block keeps its own credentials file, so the two never collide.
</Aside>
...
- **Project ID** — overrides the project. It starts out showing the `project` prop, if set; leave it blank to use the key's own `project_id`.
...
Once authenticated, the success card offers **Change project** to switch to another project the same credential can see, and **Re-authenticate** to start over. Picking a project makes the block the session default again: the session variables switch to its credential, account, and new project together, and the block keeps the region and zone it authenticated with. **Cancel** in a picker opened by **Change project** returns to the success card and keeps the current project; nothing is withdrawn. If the project list cannot be loaded, the picker shows why (*"Could not list projects: …"*) rather than an empty list.
...
If a credential is found but does not validate, the block says so — *"Invalid credentials detected: Application Default Credentials are invalid or expired"*, followed by the underlying reason in parentheses (for example, a file that does not exist) — and falls through to manual authentication.
...
Within a single Runbooks session, GoogleAuth keeps `CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE` in sync every time it authenticates — including switching from a file-backed credential to an access token, or back — so re-authenticating never leaves a stale value from an earlier credential *this block itself* set. It is not exempt from the caveat above, though: a value already present in the shell Runbooks was launched from rides into the session like any other variable, and stays there until some GoogleAuth block in this runbook authenticates and overwrites — or clears — it.
</Aside>
...
| *Invalid credentials detected: … are invalid or expired (…)* | The detected credential does not authenticate; the text in parentheses says why. For a missing or unreadable file, fix the path the variable points at. For an expired or revoked credential, refresh it (`gcloud auth application-default login`). Or authenticate manually |
...
| *Could not list projects: …* | Google refused or failed the project listing (for example, the Cloud Resource Manager API is disabled). The current authentication is untouched: **Cancel** returns to the success card. Fix the cause shown, or set the `project` prop |
...
| Commands use the wrong project | The session variables reflect the GoogleAuth block that most recently authenticated or changed project. Add `googleAuthId` to every command that must use a specific one |
