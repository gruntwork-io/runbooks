...
The `path` prop also supports remote templates using explicit protocol prefixes. See the [Template remote templates documentation](/authoring/blocks/template/#remote-templates) for details on supported protocols.

```mdx
{/* Load from a Git repository */}
<Inputs 
  id="vpc-config" 
  path="git::https://github.com/acme-corp/templates//vpc?ref=v1.0.0" 
/>
```
