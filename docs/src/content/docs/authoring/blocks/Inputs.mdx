...
- `path` (string) - Path to a directory containing a `boilerplate.yml` file, relative to the gruntbook.
...
This loads variable definitions from `templates/vpc/boilerplate.yml` relative to your gruntbook file.
...
<Command 
    id="create-vpc" 
    command="aws ec2 create-vpc --cidr-block {{ .inputs.CidrBlock }} --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value={{ .inputs.VpcName }}}]'"
    inputsId="vpc-config"
    title="Create VPC"
    successMessage="VPC {{ .inputs.VpcName }} created!"
/>
`````
...
<Command 
    id="create-repo" 
    command="gh repo create {{ .inputs.OrgName }}/{{ .inputs.RepoName }} --{{ .inputs.Visibility }}"
    inputsId="repo-settings"
    title="Create Repository"
    successMessage="Repository {{ .inputs.OrgName }}/{{ .inputs.RepoName }} created!"
...
<Check 
    id="verify-repo" 
    command="gh repo view {{ .inputs.OrgName }}/{{ .inputs.RepoName }}"
    inputsId="repo-settings"
