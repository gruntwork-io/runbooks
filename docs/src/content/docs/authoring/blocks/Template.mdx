...
Once the files have been generated, the block turns green and regenerates them automatically as the form changes. If generation fails, the error is shown above the form and the block is not green while that error is shown. If the first generation fails, the Generate button stays available so the user can retry.
