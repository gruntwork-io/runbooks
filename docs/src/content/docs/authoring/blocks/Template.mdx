...
This loads the `boilerplate.yml` file from `templates/vpc/boilerplate.yml` (relative to your gruntbook file), renders a form for the variables, and generates files when the user clicks "Generate".
...
|---------|-------------|-------------------|
| Template source | Directory with `boilerplate.yml` | Inline in gruntbook |
...
- `path` (string) - Path to the boilerplate template directory, relative to the gruntbook file. The directory must contain a `boilerplate.yml` file.
...
Gruntbooks scans template files for output references and disables the Generate button until all upstream blocks have run.
...
Here's a complete gruntbook showing Template with imported variables and validation:
