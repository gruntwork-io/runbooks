...
- `id` (string) - Unique identifier for this block. Used as `data-testid` on the root element for e2e testing.
...
Like all other blocks, `<TemplateInline>` requires an `id` prop. The `id` is rendered as `data-testid` on the root element, enabling e2e tests to use scoped locators like `page.getByTestId('block-id')`.
