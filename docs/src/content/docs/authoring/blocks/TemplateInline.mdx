...
The `<TemplateInline>` block renders Boilerplate templates directly in your gruntbook, displaying the rendered output inline. Unlike `<Template>`, which loads templates from a directory, `<TemplateInline>` lets you write template content directly in your gruntbook file.
...
|---------|-------------------|--------------|
| Template source | Inline in gruntbook | Directory with `boilerplate.yml` |
...
Gruntbooks scans the template content for output references and shows a warning until all upstream blocks have run.
...
Like all other blocks, `<TemplateInline>` requires an `id` prop. The `id` uniquely identifies the block within the gruntbook and is used for dependency resolution between blocks.
