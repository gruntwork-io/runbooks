...
If `outputPath` changes between renders, for example because it uses a [`<DirPicker>`](/authoring/blocks/dirpicker/) output, the block saves the file at the new path and removes the one it saved at the old path, along with any directories that removal leaves empty. The old file is kept if it no longer holds what the block wrote, for example because you edited it.
