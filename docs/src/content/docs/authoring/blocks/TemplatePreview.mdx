...
- _[Required]_ **Inline template content** - Write the template as a fenced code block inside the `<TemplatePreview>` tags.

- _[Optional]_ `inputsId` (string or array of strings) - ID of the Inputs block(s) to get variable values from. When multiple IDs are provided, variables are merged in order (later IDs override earlier ones). If not provided, the template renders without variable substitution.

- _[Optional]_ `outputPath` (string) - File path to display for the rendered output (e.g., `config.yaml`, `terraform/main.tf`). Note that for security reasons, the path must be a relative path without; absolute paths and directory traversal attempts are blocked
...
|---------|--------------------|---------------------------------|
| Generates files | Preview only | Saves to workspace |
| Template location | Inline only | Can reference directories |
| Use case | Show what will be generated | Actually generate files |

<Aside type="note">
Use `<TemplatePreview>` when you want to show users what files will look like. Use `<BoilerplateInputs>` when you need to actually create files in the user's workspace.
</Aside>
