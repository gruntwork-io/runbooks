...
The `<TfModule>` block parses an OpenTofu/Terraform module at runtime, dynamically renders a form for all its input variables, and publishes the collected values so [`<TemplateInline>`](/authoring/blocks/templateinline/) and [`<Template>`](/authoring/blocks/template/) blocks can generate **any** output format — Terragrunt HCL, raw Terraform, YAML configs, or anything else.
...
**`<TfModule>`** parses `.tf` files at runtime and auto-generates a form from the module's variables. It provides a `_module` namespace with source, metadata, `inputs`, and `hcl_inputs` — enabling dynamic iteration over all variables without knowing their names upfront. Use it when you want to generate config files from an existing OpenTofu module.
...
_module:
  ...
  folder_name: "vpc"
  readme_title: "VPC Module"
  output_names: ["vpc_id", "subnet_ids", "nat_gateway_ip"]
  resource_names: ["aws_vpc.this", "aws_subnet.public", "aws_nat_gateway.this"]
  inputs:
    vpc_name: "my-vpc"
...
| `_module.folder_name` | Name of the module's containing directory (e.g., `"vpc"`). |
...
You can also use `<TfModule>` with [`<Template>`](/authoring/blocks/template/) when you have a Boilerplate template directory. The Template imports TfModule values via `inputsId`, and the template files get full access to the `_module` namespace.

This is useful when you need multi-file generation from a template directory — for example, generating a `terragrunt.hcl` alongside a `README.md` and a CI config from a single set of module variables.

`````mdx
<TfModule id="module-vars" source="../modules/vpc" />

<Template id="vpc-scaffold" path="templates/vpc-scaffold" inputsId="module-vars" />
`````

Inside `templates/vpc-scaffold/terragrunt.hcl`:
...
Inside `templates/vpc-scaffold/README.md`:
...
<Aside type="note">
When Template imports from TfModule, all TfModule values — including `_module` and every individual variable — become **imported-only** variables. They are passed through to the template engine but are not shown in the Template's form. If the Template's own `boilerplate.yml` defines a variable with the same name as a TfModule variable, it becomes a **shared** variable: read-only in the form and live-synced to the TfModule value.
</Aside>
...
|---|---|---|
| Template source | Inline in the runbook | Separate directory with `boilerplate.yml` |
| Multi-file generation | One file per block | Multiple files from one directory |
| Extra variables | None | Can define additional variables in `boilerplate.yml` |
| Best for | Quick single-file previews and generation | Structured multi-file scaffolding |
