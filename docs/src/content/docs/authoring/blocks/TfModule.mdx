...
<Aside type="note">
You can also select a built-in template with `--tf-template` (e.g., `--tf-template full`) instead of providing a custom runbook. The `--tf-runbook` flag takes precedence if both are set.
</Aside>
