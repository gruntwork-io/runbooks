...
<Aside type="note">
You can also select a built-in template by passing a keyword instead of a path: `--tf-runbook full`. Available keywords: `basic` (default), `full`. To use a custom runbook directory that has the same name as a keyword, use a path prefix: `--tf-runbook ./full`.
</Aside>
