...
Variables are the keys in the global context. You reference them in templates, scripts, and block props using `{{ .inputs.VarName }}` syntax (for blocks on the render-inline path like `<Command>`, `<Check>`, and `<TemplateInline>`) or `{{ .VarName }}` (for blocks on the full render path like `<Template>`). For example:
