...
In the above example, there is an OpenTofu or Terraform module at `../modules/rds`.
...
When a rendering block imports values from an input provider via `inputsId`, three distinct types of variables are available:

- **Local variables.** These are variables defined in the rendering block (e.g., in Template's `boilerplate.yml`) but not provided by the input provider. These appear as editable fields in the rendering block's form.
- **Imported-only** — provided by the input provider but not defined in the rendering block. These are passed through to the template engine without appearing in the form. When `<TfModule>` is the provider, this includes the entire `_module` namespace.
- **Shared** — defined in both places. These appear in the rendering block's form as read-only fields that stay live-synced to the input provider's values.

This matters most when `<Template>` imports from `<Inputs>` or `<TfModule>`. For `<TemplateInline>`, `<Command>`, and `<Check>`, all imported variables are simply available in templates — there is no local form to merge with.
