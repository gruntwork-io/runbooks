...
Every runbook has a **global context** — a shared collection of key-value pairs that blocks use to communicate. The keys are variables. Blocks publish values into the global context, and other blocks read from it.

There are two ways values enter the global context:
...
- **Outputs** — Blocks that execute scripts (like `<Command>` and `<Check>`) can publish results to the global context. Downstream blocks reference them via the [`_blocks` namespace](#block-outputs).

## Variables

Variables are the keys in the global context. You reference them in templates, scripts, and block props using `{{ .VarName }}` syntax:
