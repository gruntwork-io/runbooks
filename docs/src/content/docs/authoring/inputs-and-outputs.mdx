...
Every gruntbook has a **global context,** which is the shared collection of key-value pairs that blocks use to communicate. The keys are variables. Blocks can write values to the global context, or read values from the global context.

There are two ways values are written to the global context:

- **Inputs:** [Input providers](#input-providers) (like `<Inputs>`, `<TfModule>`, and `<Template>`) collect values from end users via web forms and publish them to the global context. Consumer blocks pull in those values via the [`inputsId` prop](#wiring-blocks-with-inputsid).
- **Outputs:** Several block types (e.g., `<Command>`, `<Check>`, `<TfModule>`, `<DirPicker>`) publish results to the global context. Downstream blocks reference them via the [`outputs` namespace](#block-outputs).

## Variables

Variables are the keys in the global context. You reference them in templates, scripts, and block props using `{{ .inputs.VarName }}` syntax for inline rendering blocks (`<Command>`, `<Check>`, and `<TemplateInline>`) or `{{ .VarName }}` for template files used by `<Template>`. For example:

```mdx
<Command
  id="create-account"
  command="aws organizations create-account --email {{ .inputs.Email }} --account-name {{ .inputs.AccountName }}"
/>
```

When this block runs, Gruntbooks resolves `{{ .inputs.Email }}` and `{{ .inputs.AccountName }}` from the global context, producing a command like:
...
Under the hood, Gruntbooks uses [Gruntwork Boilerplate](/authoring/boilerplate/) as its template engine. This means that variables work exactly like Boilerplate variables:
...
Gruntbooks supports two template syntaxes for referencing variables:
...
<Aside type="caution">
**Reserved variable names:** `inputs` and `outputs` are reserved keywords and cannot be used as variable names in your `boilerplate.yml` files.
</Aside>

## Input Providers

An **input provider** is a block that collects variable values and makes them available to other blocks. Let's look at the input providers that are available in Gruntbooks.
...
In the above example, there is an OpenTofu or Terraform module at `../modules/rds`. TfModule also supports `source="."` for [colocated gruntbooks](/authoring/blocks/tfmodule/#colocated-gruntbooks-source) (gruntbook lives alongside the `.tf` files) and `source="::cli_gruntbook_source"` for [generic gruntbooks](/authoring/blocks/tfmodule/#dynamic-source-from-cli-source) that accept any module URL from the CLI.
...
<Command inputsId="config" command="mkdir {{ .inputs.ProjectName }}" />
`````
...
<Command
  ...
  command="deploy --env {{ .inputs.Environment }} --name {{ .inputs.AppName }}"
/>
```
...
`````mdx
<Command command='echo "Hello, {{ .inputs.Name }}!"'>
  <Inputs id="greeting">
  ```yaml
  variables:
    - name: Name
...
It gets more interesting with `<Template>`, because a Template has its own `boilerplate.yml` that can *also* define variables. When a Template imports from an input provider, Gruntbooks compares the two sets of variables and handles each one based on where it's defined:
...
[`<Command>`](/authoring/blocks/command/) and [`<Check>`](/authoring/blocks/check/) blocks produce outputs by writing `key=value` pairs to the file specified by the `$GRUNTBOOK_OUTPUT` environment variable:
...
echo "region=us-west-2" >> "$GRUNTBOOK_OUTPUT"
```
...
You don't need to define the `$GRUNTBOOK_OUTPUT` environment variable in your script. Gruntbooks will automatically create it for you.
...
<Aside type="note">
**ID naming:** Block IDs can use hyphens or underscores (e.g., `id="create-account"` or `id="create_account"`). In template syntax, always use underscores. Go templates interpret hyphens as subtraction. IDs that normalize to the same value (e.g., `create-account` and `create_account`) cannot coexist in the same gruntbook.
</Aside>
...
```bash
gruntbooks open https://github.com/gruntwork-io/runbooks/tree/main/testdata/feature-demos/block-outputs
```
