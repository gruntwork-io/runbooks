...
If the built-in templates don't fit your needs, create your own. A custom template is a **local** directory containing a `runbook.mdx` that uses [`<TfModule>`](/authoring/blocks/tfmodule/) with `source="::cli_runbook_source"`.
