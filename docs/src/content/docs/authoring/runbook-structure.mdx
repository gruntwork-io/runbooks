...
|--------|---------|---------|
| `assets/` | Images, diagrams, videos, and other static files | Markdown image syntax and `<img>`, `<video>`, `<audio>` tags |
...
<video src="./assets/demo.mp4" poster="./assets/demo-poster.png" controls />
```

Asset paths must start with `./assets/`. Runbooks loads them from the runbook's `assets/` folder whether they appear in markdown image syntax or in the `src` of an `<img>`, `<video>`, `<audio>` or `<source>` tag (and a `<video>`'s `poster`). Write the path as a plain string: `src={"./assets/demo.mp4"}` is left as written and won't load.
