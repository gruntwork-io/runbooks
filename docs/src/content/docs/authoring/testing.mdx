...
| `<TemplateInline>` | Auto-generated from `outputPath` | `outputPath="config/settings.json"` → `template-settings-980db14c` |
| `<AwsAuth>` | Explicit `id` prop | `<AwsAuth id="aws-auth" ...>` → `aws-auth` |
| `<GitHubAuth>` | Explicit `id` prop | `<GitHubAuth id="github-auth" ...>` → `github-auth` |

**TemplateInline ID Format:**

Since `<TemplateInline>` blocks don't have an explicit `id` prop, IDs are auto-generated using the format:

```
template-{basename}-{hash}
```

Where `{basename}` is the filename without extension (e.g., `settings` from `config/settings.json`), and `{hash}` is an 8-character SHA256 hash of the full `outputPath`. The hash ensures unique IDs when multiple blocks have the same filename in different directories—without it, both `config/settings.json` and `other/settings.json` would generate `template-settings`, causing ID conflicts.

| `outputPath` | Generated ID |
|--------------|--------------|
| `config.yaml` | `template-config-a1b2c3d4` |
| `config/settings.json` | `template-settings-980db14c` |
| `other/settings.json` | `template-settings-7f3e2a1b` |

If no `outputPath` is specified, IDs are generated sequentially using the format `template-inline-1`, `template-inline-2`, etc., based on the order the blocks appear in the runbook.

:::tip
Run `runbooks test init` to automatically discover all block IDs. The generated test config includes comments showing which `outputPath` each TemplateInline ID corresponds to.
:::
...
To avoid conflicts with your local AWS credentials, use the `testPrefix` option in `detectCredentials` to specify a prefix for test-only environment variables:
...
This will check for the following environment variables in tests:
...
You can also specify different prefixes for runtime (what end users will experience) vs tests:
...
- **Runtime**: Checks `PROD_AWS_ACCESS_KEY_ID`, etc.
- **Tests**: Checks `CI_AWS_ACCESS_KEY_ID`, etc.
...
Similar to AwsAuth, you can specify a `testPrefix` to use custom environment variables for tests:
...
This will check for `CI_GITHUB_TOKEN` and `CI_GH_TOKEN` in tests.
