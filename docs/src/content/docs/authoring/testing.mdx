...
settings:
  use_temp_output: true  # Generate files to temp directory
...
|---------|---------|-------------|
| `use_temp_output` | `true` | Generate files to a temporary directory (automatically cleaned up) |
...
5. **Keep tests fast**: Use `use_temp_output: true` and avoid slow external dependencies when possible.
