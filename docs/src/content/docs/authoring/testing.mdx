---
title: Testing Gruntbooks
description: How to write and run automated tests for your gruntbooks.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Gruntbooks includes a built-in testing framework that lets you validate that your gruntbooks work correctly.

To run a test, you define a YAML test configuration file alongside your gruntbook. Then call `gruntbooks test /path/to/gruntbook` or `gruntbooks test ./...` to run the tests. Test are meant to run locally or in CI.

## Quick Start

1. Generate a test configuration for your gruntbook:

```bash
gruntbooks test init ./my-gruntbook
```

This creates `gruntbook_test.yml` with reasonable defaults based on your gruntbook's blocks. You can edit the file to customize the tests if needed.

2. Run the tests:

```bash
gruntbooks test ./my-gruntbook
```

## Test Configuration

Tests are defined in a `gruntbook_test.yml` file located in the same directory as your `gruntbook.mdx` file. Here's an example of a test configuration file:

:::note[Legacy filename]
For backward compatibility, Gruntbooks also accepts the legacy filename `runbook_test.yml` (with a deprecation warning). Prefer `gruntbook_test.yml` for new gruntbooks.
:::
...
settings:
  ...
  # working_dir: .              # Alternative: use gruntbook's directory
  # output_path: generated      # Where to write generated files (relative to working_dir)
  timeout: 5m            # Test timeout
  parallelizable: true   # Can run in parallel with other gruntbooks
...
| `tests` | array | Yes | Array of gruntbook tests to run. At least one test case is required. |
...
| `working_dir` | string | current directory | Base directory for script execution and file generation. Use `.` for the gruntbook's directory. Ignored when `use_temp_working_dir` is `true`. |
...
| `parallelizable` | boolean | `true` | Whether this gruntbook's tests can run in parallel with tests from other gruntbooks. |
...
:::tip
Run `gruntbooks test init` to automatically discover all block IDs in your gruntbook.
:::
...
For gruntbooks with `<AwsAuth>` blocks, the test framework checks for AWS credentials in environment variables and injects them into the session for dependent blocks to use.
...
tests:
  - name: happy-path
    steps:
      - block: aws-auth
        env_prefix: GRUNTBOOKS_TEST_
        expect: success
```

This tells the test executor to check for the following environment variables:

- `GRUNTBOOKS_TEST_AWS_ACCESS_KEY_ID`
- `GRUNTBOOKS_TEST_AWS_SECRET_ACCESS_KEY`
- `GRUNTBOOKS_TEST_AWS_SESSION_TOKEN`
- `GRUNTBOOKS_TEST_AWS_REGION`

If no prefixed credentials are found, the executor falls back to standard env vars (`AWS_ACCESS_KEY_ID`, etc.).

The `env_prefix` is configured in the test config file, not in the gruntbook MDX. This keeps test infrastructure concerns separate from the runtime `detectCredentials` behavior that end users experience.
...
- name: Run gruntbook tests
  run: gruntbooks test ./gruntbooks/...
  env:
    GRUNTBOOKS_TEST_AWS_ACCESS_KEY_ID: ${{ env.AWS_ACCESS_KEY_ID }}
    GRUNTBOOKS_TEST_AWS_SECRET_ACCESS_KEY: ${{ env.AWS_SECRET_ACCESS_KEY }}
    GRUNTBOOKS_TEST_AWS_SESSION_TOKEN: ${{ env.AWS_SESSION_TOKEN }}
    GRUNTBOOKS_TEST_AWS_REGION: ${{ env.AWS_REGION }}
```

## Testing GitHubAuth Blocks

For gruntbooks with `<GitHubAuth>` blocks, the test framework checks for GitHub tokens in environment variables.

By default, the test framework checks in this order:

1. `GRUNTBOOKS_GITHUB_TOKEN` (recommended for tests)
2. `GITHUB_TOKEN`
3. `GH_TOKEN`
...
- name: Run gruntbook tests
  run: gruntbooks test ./gruntbooks/...
  env:
    GRUNTBOOKS_GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```

## Running Tests

When you run a gruntbook test, the test framework will:
- Run the gruntbook in a temporary directory
- Capture the output of any scripts or generated files
- Validate the output of the gruntbook against the assertions in your test configuration file, if any
...
When scripts run in tests, they execute without a PTY (pseudo-terminal), so you may see less output than when running the gruntbook interactively. For example, `git clone` shows progress messages ("Receiving objects: 47%...") only when connected to a terminal. In gruntbook tests, you'll see just the essential output. If you do need verbose output for debugging, many commands have flags to force it (e.g., `git clone --progress`), or just open the gruntbook and manually run the script.
:::

If a gruntbook test fails, you will see additional details about which block failed and why.

### Single Gruntbook

You can run a test on a single gruntbook.

```bash
# Run all tests for a gruntbook
gruntbooks test ./my-gruntbook

# Run a specific test case
gruntbooks test ./my-gruntbook --test happy-path

# Verbose output
gruntbooks test ./my-gruntbook -v
```

### Multiple Gruntbooks

Use the `...` glob pattern to discover and test all gruntbooks:

```bash
# Test all gruntbooks in a directory tree
gruntbooks test ./gruntbooks/...

# Control parallel execution
gruntbooks test ./gruntbooks/... --max-parallel 4
```
...
```bash
gruntbooks test ./gruntbooks/... --output junit --output-file results.xml
```

## Out-of-Order Testing

In some cases, for testing purposes, you may wish to execute blocks in an order different than how they're defined in the gruntbook. You can do this by explicitly specifying the order of the blocks in the test configuration file.

Keep in mind, though, that when new blocks are added to the gruntbook, they will not be included in the test unless you explicitly add them to the test configuration file.
...
```yaml
# .github/workflows/test-gruntbooks.yml
name: Test Gruntbooks
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Set up gruntbooks
        run: |
          curl -sL https://github.com/gruntwork-io/runbooks/releases/latest/download/gruntbooks_linux_amd64 -o gruntbooks
          chmod +x gruntbooks
      ...
      - name: Run gruntbook tests
        run: ./gruntbooks test ./gruntbooks/... --output junit --output-file results.xml
        env:
          # Map OIDC credentials to GRUNTBOOKS_TEST_ prefix
          GRUNTBOOKS_TEST_AWS_ACCESS_KEY_ID: ${{ env.AWS_ACCESS_KEY_ID }}
          GRUNTBOOKS_TEST_AWS_SECRET_ACCESS_KEY: ${{ env.AWS_SECRET_ACCESS_KEY }}
          GRUNTBOOKS_TEST_AWS_SESSION_TOKEN: ${{ env.AWS_SESSION_TOKEN }}
          GRUNTBOOKS_TEST_AWS_REGION: ${{ vars.AWS_REGION }}
...
test-gruntbooks:
  stage: test
  variables:
    GRUNTBOOKS_TEST_AWS_ACCESS_KEY_ID: $AWS_ACCESS_KEY_ID
    GRUNTBOOKS_TEST_AWS_SECRET_ACCESS_KEY: $AWS_SECRET_ACCESS_KEY
    GRUNTBOOKS_TEST_AWS_REGION: $AWS_REGION
  script:
    - curl -sL https://github.com/gruntwork-io/runbooks/releases/latest/download/gruntbooks_linux_amd64 -o gruntbooks
    - chmod +x gruntbooks
    - ./gruntbooks test ./gruntbooks/... --output junit --output-file results.xml
...
Gruntbooks that interact with external services (GitHub, AWS, Terraform, etc.) require special testing strategies. We recommend a tiered approach.
...
Dry-run mode tests that script logic works correctly without making any external calls. They leverage the fact that the script was written to support a `GRUNTBOOK_DRY_RUN` environment variable.

Use the `GRUNTBOOK_DRY_RUN` environment variable pattern to test script logic without side effects. Note that the `GRUNTBOOK_DRY_RUN` environment variable is not special in any way, it is just a convention. You can use any environment variable you want to trigger the dry-run mode.
...
tests:
  - name: integration-test
    ...
    env:
      ...
      # So no need to set them in the gruntbook_test.yml file.
...
2. **Test happy paths first**: Ensure your gruntbook works when everything goes right.
...
8. **Use tiered testing**: For gruntbooks with external dependencies, use dry-run mode in CI and save integration tests for manual/scheduled runs.

Finally, note that tests are not free to write or maintain. But it is generally worth the investment to know that your gruntbooks work as expected. Choose accordingly how much to invest in testing your gruntbooks.
