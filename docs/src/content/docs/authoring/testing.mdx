...
Runbooks includes a built-in testing framework that lets you validate your runbooks work correctly. Tests are defined in YAML files alongside your runbooks and can be run locally or in CI.
...
This creates `runbook_test.yml` with reasonable defaults based on your runbook's blocks.

2. Edit the generated file to add your test values and assertions.
...
Tests are defined in `runbook_test.yml` files located in the same directory as your `runbook.mdx`.
...
Assertions validate the test results. They run after all steps complete (or immediately after a step if defined inline).
...
In CI, set the environment variables before running tests:
...
For runbooks without `prefilledCredentials`, skip the AwsAuth block:
...
Test that blocks correctly enforce dependencies by checking for blocked status:
...
Test that templates generate correct files without any external calls:
...
Use the `RUNBOOK_DRY_RUN` environment variable pattern to test script logic without side effects. Note that `RUNBOOK_DRY_RUN` is not special in any way, it is just a convention. You can use any environment variable you want to trigger the dry-run mode.
