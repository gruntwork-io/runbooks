...
| `env` | map | No | Environment variables to set for all blocks and `script` assertions in this test. Keys and values are strings. |
...
Both kinds of action run in bash from the output directory (`<working_dir>/<output_path>`), which is created first if no block has generated files yet. A `path` script is read relative to the runbook's directory. A failing cleanup action doesn't fail the test and doesn't stop the remaining actions, but it is printed as a warning.
