...
The test runner is a standalone Node.js CLI included in the Runbooks repository. After building the project (`just build`), invoke it as `node dist/cli/index.js test`. The examples below use `runbooks-cli` as shorthand for this invocation.
...
```bash
runbooks-cli test init ./my-runbook
```
...
:::tip
Run `runbooks-cli test init` to automatically discover all block IDs in your runbook.
:::
...
runbooks-cli test ./runbooks/... --max-parallel 4
```
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run runbook tests
        run: node dist/cli/index.js test ./runbooks/... --output junit --output-file results.xml
        env:
          RUNBOOKS_TEST_AWS_ACCESS_KEY_ID: ${{ env.AWS_ACCESS_KEY_ID }}
          RUNBOOKS_TEST_AWS_SECRET_ACCESS_KEY: ${{ env.AWS_SECRET_ACCESS_KEY }}
          RUNBOOKS_TEST_AWS_SESSION_TOKEN: ${{ env.AWS_SESSION_TOKEN }}
...
test-runbooks:
  ...
  before_script:
    ...
    - electron-vite build
  script:
    - node dist/cli/index.js test ./runbooks/... --output junit --output-file results.xml
  artifacts:
    reports:
      junit: results.xml
```
