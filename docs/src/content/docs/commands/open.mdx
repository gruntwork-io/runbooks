...
| GitHub permalink (commit) | `https://github.com/org/repo/tree/<commit-sha>/runbooks/setup-vpc` |
| OpenTofu GitHub shorthand | `github.com/org/repo//path?ref=v1.0` |
| OpenTofu GitLab shorthand | `gitlab.com/group/subgroup/repo//path?ref=v1.0` |
| OpenTofu git::https | `git::https://github.com/org/repo.git//path?ref=main` |
| OpenTofu pinned to a commit | `github.com/org/repo//path?ref=<commit-sha>` |

A ref can be a branch, a tag, or a commit SHA. An abbreviated SHA needs at least 7 characters, and the commit must be reachable from a branch or tag (a commit that only exists on a pull request head cannot be opened). In the OpenTofu formats, `//path` is optional (without it the runbook is at the repo root), and without `?ref` the default branch is used. Credentials in the URL (`https://user:token@...`) are not supported; see [Authentication for private repos](#authentication-for-private-repos) instead.
