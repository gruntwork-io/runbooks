...
A ref can be a branch, a tag, or a commit SHA (full or abbreviated). In the OpenTofu formats, `//path` is optional (without it the runbook is at the repo root), and without `?ref` the default branch is used.
