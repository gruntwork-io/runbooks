...
```bash
runbooks serve <source> [flags]
```

## Arguments

- `<source>` - A local path or remote URL pointing to a directory containing `runbook.mdx`, or to the file itself. See [`runbooks open`](/commands/open) for supported remote URL formats.
