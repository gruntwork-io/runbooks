...
When you use a Gruntbook, you'll often work with files, whether they're freshly generated from templates, or cloned from a git repository. The **files workspace** (labeled just "Files" in the Gruntbooks UI) is the panel on the right side of the Gruntbooks UI where all of these files live.
...
##### Why use them
You want the gruntbook to generate new files and expect the user will then perform some _out-of-band action_ with the files, such as manually integrating them into a separate project.
...
##### Why use them
You want the user to `git clone` a repository and then create, modify, or delete files in that repo using subsequent blocks in the gruntbook. Presumably, after git cloning the files and making changes, the gruntbook will either `git push` the changes or open a pull request on them.
...
<Aside type="tip">
If a gruntbook only produces generated files (no git clone), you'll only see the "Generated files" tab. If it only clones a repository, you'll only see the "Repository" tab. When both are present, you can switch between them using the context bar at the top of the workspace.
</Aside>

## Generated files

Generated files are new files that the gruntbook creates on your local machine. There are two main ways they get created:
...
By default, generated files are written to a `generated/` folder in the working directory (the directory you run the `gruntbooks` command from):
...
gruntbooks open my-gruntbook --working-dir /path/to/project

# Write to a custom subfolder within the working directory
gruntbooks open my-gruntbook --output-path ./infrastructure

# Use an isolated temporary directory (auto-cleaned on exit)
gruntbooks open my-gruntbook --working-dir=::tmp
```

<Aside type="tip">
The `--working-dir=::tmp` keyword is useful for sandboxed execution where you don't want generated files to persist after you close the gruntbook.
</Aside>
...
Repository files come from a git repository that was cloned by a [GitClone](/authoring/blocks/gitclone/) block in the gruntbook. When the clone completes, the workspace automatically switches to the **Repository** tab to show the cloned files.
...
The **Changed files** tab shows a diff view of any files that have been modified since the repository was cloned. This is useful when subsequent blocks in the gruntbook (like Command or Template blocks with `target="worktree"`) write files into the cloned repository. You'll see:
...
If a gruntbook clones more than one repository (using multiple GitClone blocks), a dropdown appears in the workspace header letting you switch between them.

## Security: Where files can be read and written

Gruntbooks enforces rules about where files can be read and written:
...
Gruntbooks blocks writes to system-critical directories:
...
Now that you understand how the files workspace works, explore the [use cases](/intro/use_cases/) where Gruntbooks shines.
