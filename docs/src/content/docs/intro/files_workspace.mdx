...
The files are stored in a `generated/` folder on the user's local machine, inside the working directory (the folder that contains `runbook.mdx`).
...
##### Where they're stored
The files are stored in a local path (relative to the working directory, the folder that contains `runbook.mdx`) specified in the [\<GitClone\>](/authoring/blocks/gitclone/) block.
...
Generated files are written to a `generated/` folder in the working directory, which is the folder that contains `runbook.mdx`:
...
For a [remote runbook](/commands/open#remote-sources), the working directory is inside a temporary clone, so its generated files are deleted when you quit Runbooks.
