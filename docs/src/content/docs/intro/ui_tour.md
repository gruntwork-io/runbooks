---
title: UI Tour
sidebar:
   order: 4
---

On this page, we'll do a live walkthrough of the **Runbook consumer** experience when a user opens the [demo-runbook-lambda runbook](https://github.com/gruntwork-io/runbooks/tree/main/testdata/demo-runbook-lambda).

To learn more about authoring runbooks, see [write your first runbook](../write_your_first_runbook).

<div style="position: relative; padding-bottom: 56.25%; height: 0;">
  <iframe src="https://www.loom.com/embed/0848381b1e174670895e3228a69b865a" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"></iframe>
</div>

## Next

Now that you understand how Runbooks work, let's learn about their real-world use cases.
