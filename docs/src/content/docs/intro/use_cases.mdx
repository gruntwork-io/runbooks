...
Gruntbooks enables infrastructure experts to scale their expertise. Here are the most common practical scenarios where infrastructure expertise winds up being a painful bottleneck:
...
Gruntbooks capture a subject matter expert's knowledge and experience in a gruntbook and enable app developers to easily launch new infrastructure that follows your organization's best practices. For example:
...
There are well-established solutions for [1] (like defining an OpenTofu module), and [2] (like calling a cloud API). Gruntbooks can serve as an excellent way to combine [1] and [2] into [3], a configurable, powerful UI that asks users to configure their desired new account/project/subscription (e.g. with an email, name, description), calls the relevant API to provision the account/project/subscription, and then generates the necessary code to install the baseline.

Because Gruntbooks can ask for a user-entered value like an AWS account ID, you can plumb this value through your generated code and scripts to accomplish whatever you need.

You could even have the gruntbook provision SSO access, teach the user how to use the newly provisioned resources, guide them to review policies, or give them customized links to access their account, all without coding any custom user interface.

## Streamline internal processes

Many internal processes are captured as static documentation. But with Gruntbooks, you can complete the actual process by the time you're doing reading the Gruntbook! For example:
...
Traditional documentation describes your module, but Gruntbooks can actually collect configuration from the user, generate the module code, and validate that the module was created correctly. For example:
...
You might define one Gruntbook for each of these, or a single Gruntbook that can create any of these configurations.

# Next

You're almost done with the intro! As a last step, let's see how [Gruntbooks compares to alternatives](/intro/gruntbooks_vs_other/).