Previously, opening a gruntbook required having the repository cloned locally. If someone shared a GitHub link to a gruntbook, you had to clone the repo, find the right directory, and run `gruntbooks open ./path/to/gruntbook`. This was friction for gruntbook consumers who just want to run a gruntbook they were sent a link to.

This PR adds remote gruntbook support — you can now pass a GitHub or GitLab URL directly to `gruntbooks open`, `gruntbooks watch`, or `gruntbooks serve`, and the tool downloads just what it needs and serves it immediately.
...
gruntbooks open https://github.com/org/repo/blob/main/gruntbooks/setup-vpc/gruntbook.mdx
...
When the URL points to a subdirectory within a repo, `downloadRemoteGruntbook` in `cmd/remote_open.go` uses `git sparse-checkout` to clone only that directory — avoiding downloading the entire repository.
...
- **Automatic `--working-dir-tmp`.** When opening a remote gruntbook, if no `--working-dir` is specified, the working directory automatically becomes a temp directory (equivalent to `--working-dir-tmp`). This prevents generated files from being written into the cloned gruntbook directory.
...
- **Copy local path button.** When viewing a remote gruntbook, a folder icon appears next to the URL in the header. Clicking it copies the local temp directory path to the clipboard, with a tooltip showing the full path.
- **Mobile-friendly.** On small viewports, the `https://` prefix is stripped for a more compact display.

## `GruntbookConfig` struct

The three booleans/strings previously threaded through handler functions (`gruntbookPath`, `isWatchMode`, `useExecutableRegistry`) are now bundled into a `GruntbookConfig` struct in `api/file.go`, with the new `RemoteSourceURL` field added alongside them:
...
All three server entry points (`StartServer`, `StartBackendServer`, `StartServerWithWatch`) now accept a `remoteSourceURL` parameter and construct a `GruntbookConfig` to pass to `HandleGruntbookRequest`.
...
`api/path_validation.go` extracts a new `ValidateAbsolutePathInDir(path, dir)` function from the existing `ValidateAbsolutePathInCwd`. The old function now delegates to the new one with `os.Getwd()`. This is needed because remote gruntbooks use a temp working directory that differs from the process CWD, and `generated_files.go` needs to validate paths against that directory.
...
### `ResolveGruntbookPath` simplified

`api/watcher.go`: `ResolveGruntbookPath` no longer tries `gruntbook.md` as a fallback — it only looks for `gruntbook.mdx`. The error message now specifies what was expected.

### Test refactors

`api/file_test.go` is significantly reduced (from ~700 lines to ~230) by extracting test helpers (`gruntbookRequest`, etc.) that eliminate repeated router/request boilerplate. `cmd/remote_open_test.go` covers URL parsing integration, auth hint generation, and clone error classification.
