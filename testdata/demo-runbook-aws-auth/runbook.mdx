...
<AwsAuth
  id="aws-auth-sandbox"
  title="Authenticate to AWS: Sandbox"
...
<AwsAuth
  id="aws-auth-sandbox2"
  title="Authenticate to AWS: Sandbox2"
  description="Choose your preferred authentication method. Your credentials will be used for subsequent AWS operations in this runbook."
  defaultRegion="us-west-2"
  ssoRegion="us-west-2"
  enableCredentials={true}
...
<Check
  ...
  command="echo $AWS_REGION && aws s3 ls"
  awsAuthId="aws-auth-sandbox"
...
<Check
  id="list-buckets-2"
  title="List S3 Buckets 2"
  description="Lists all S3 buckets in your AWS account"
  command="echo $AWS_REGION && aws s3 ls"
  awsAuthId="aws-auth-sandbox2"
  successMessage="S3 buckets listed successfully!"
  failMessage="Failed to list S3 buckets. Make sure you have the necessary permissions."
/>
...
Once authenticated, you can run any AWS CLI command using the `<Command>` block with `awsAuthId` set to your AwsAuth block ID:
...
When you use `awsAuthId="aws-auth"` on a `<Command>` or `<Check>` block, the following environment variables are automatically available:
