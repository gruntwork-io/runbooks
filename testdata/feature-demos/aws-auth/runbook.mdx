...
### From Environment Variables (CI/CD Mode)

When running in CI/CD environments where AWS credentials are already available (e.g., via OIDC, IAM roles, or aws-vault), you can pre-fill the AwsAuth block from environment variables:

<AwsAuth
  id="aws-auth-from-env"
  title="AWS Access (from environment)"
  description="Credentials are automatically loaded from AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, etc."
...
<AwsAuth
  ...
  description="Uses credentials from the assume-role script above"
  prefilledCredentials={{ type: "block", blockId: "assume-cross-account-role" }}
  defaultRegion="us-east-1"
/>
