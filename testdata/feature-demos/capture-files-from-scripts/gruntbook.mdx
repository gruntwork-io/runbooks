...
This gruntbook demonstrates how scripts can generate files that are automatically captured
and saved to the `/generated` folder. Files written to the special `$GENERATED_FILES` 
directory are collected after script execution and appear in the Generated Files panel.

## How It Works

1. Every script has access to the `$GENERATED_FILES` environment variable
...
or any other artifacts as part of your gruntbook workflow.
...
will write files to `$GENERATED_FILES`, which will be captured and copied to `/generated`.
...
The `$GENERATED_FILES` feature enables gruntbooks to:
