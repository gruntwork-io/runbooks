...
if [ -z "${GENERATED_FILES:-}" ]; then
    ...
    echo "This variable is automatically provided by the Gruntbooks server."
    exit 1
fi
...
echo "📁 Output directory: $GENERATED_FILES"
echo "📦 Project: $PROJECT_NAME ($ENVIRONMENT)"
echo ""

# Create directories
mkdir -p "$GENERATED_FILES/tofu"
...
cat > "$GENERATED_FILES/tofu/variables.tf" << 'EOF'
# =============================================================================
# Variables for the infrastructure
# Generated by Gruntbooks
# =============================================================================
...
variable "project_name" {
  ...
  type        = string
EOF
echo "  default     = \"${PROJECT_NAME}\"" >> "$GENERATED_FILES/tofu/variables.tf"
cat >> "$GENERATED_FILES/tofu/variables.tf" << 'EOF'
}
...
variable "environment" {
  ...
  type        = string
EOF
echo "  default     = \"${ENVIRONMENT}\"" >> "$GENERATED_FILES/tofu/variables.tf"
cat >> "$GENERATED_FILES/tofu/variables.tf" << 'EOF'
}
...
variable "aws_region" {
  ...
  type        = string
EOF
echo "  default     = \"${AWS_REGION}\"" >> "$GENERATED_FILES/tofu/variables.tf"
cat >> "$GENERATED_FILES/tofu/variables.tf" << 'EOF'
}
...
variable "team_email" {
  ...
  type        = string
EOF
echo "  default     = \"${TEAM_EMAIL}\"" >> "$GENERATED_FILES/tofu/variables.tf"
cat >> "$GENERATED_FILES/tofu/variables.tf" << 'EOF'
}
...
variable "app_port" {
  ...
  type        = number
EOF
echo "  default     = ${APP_PORT}" >> "$GENERATED_FILES/tofu/variables.tf"
cat >> "$GENERATED_FILES/tofu/variables.tf" << 'EOF'
}
...
variable "replica_count" {
  ...
  type        = number
EOF
echo "  default     = ${REPLICA_COUNT}" >> "$GENERATED_FILES/tofu/variables.tf"
cat >> "$GENERATED_FILES/tofu/variables.tf" << 'EOF'
}
EOF
...
cat > "$GENERATED_FILES/tofu/main.tf" << 'EOF'
# =============================================================================
...
cat > "$GENERATED_FILES/tofu/outputs.tf" << 'EOF'
# =============================================================================
# Outputs
# Generated by Gruntbooks
# =============================================================================
...
find "$GENERATED_FILES" -type f | sort | while read file; do
    relpath="${file#$GENERATED_FILES/}"
    size=$(wc -c < "$file" | tr -d ' ')
