...
if ! touch "$GRUNTBOOK_OUTPUT" 2>/dev/null; then
    echo "Error: GRUNTBOOK_OUTPUT ('$GRUNTBOOK_OUTPUT') is not writable." >&2
    exit 1
fi
...
if [[ -z "$PROJECT_ID" ]]; then
    echo "Error: Failed to generate PROJECT_ID. GRUNTBOOK_OUTPUT='$GRUNTBOOK_OUTPUT'" >&2
    exit 1
fi
...
echo "project_id=$PROJECT_ID" >> "$GRUNTBOOK_OUTPUT"
echo "status=created" >> "$GRUNTBOOK_OUTPUT"
