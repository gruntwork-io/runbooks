...
This gruntbook demonstrates the pseudo-TTY (PTY) support feature in Gruntbooks.
...
With PTY support, Gruntbooks executes commands in a pseudo-terminal, so tools behave as if running in a real terminal.
...
<Command 
  ...
  args={["{{ .inputs.repo_url }}", "{{ .inputs.clone_dir }}"]}
/>
...
Gruntbooks uses pseudo-TTY (PTY) by default to execute commands on Unix-like systems:
