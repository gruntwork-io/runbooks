...
This test demonstrates the `usePty` prop. When set to `false`, scripts run using pipes instead of PTY.
This is useful when PTY causes issues with certain scripts.

### With PTY (default)

<Command 
  id="color-demo-pty" 
  title="Color Output (PTY mode)"
  path="scripts/color-demo.sh"
  usePty={true}
  description="Colors are processed through PTY"
/>

### Without PTY (pipes mode)

<Command 
  id="color-demo-pipes" 
  title="Color Output (Pipes mode)"
  path="scripts/color-demo.sh"
  usePty={false}
  description="Same script, but running with pipes instead of PTY"
/>
