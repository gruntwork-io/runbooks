...
Congratulations on your new Gruntwork purchase. To bootstrap your new best-practice AWS multi-account setup, we'll use a collection of **Gruntbooks** (this one being the first) to both bootstrap your infrastructure and teach you what's happening at every step of the way.
...
You'll see the IaC architecture shortly in the next Gruntbook.
...
- **Purpose:** Store your organization's officially approved infrastructure patterns, including OpenTofu/Terraform modules, Terragrunt units, Terragrunt stacks, Gruntbooks (like this one!), Boilerplate templates, and policies-as-code. When your app teams launch infrastructure, they should be pulling resources from this catalog repo.
- **Access:** This repo will have access to the "shared" AWS accounts, such as the logs account, the security account, and the shared account.
- **Users:** This repo is typically restricted to a small group of trusted users.
...
Now that you understand the repos and how they work together, let's move on to the next gruntbook where we'll start creating the actual infrastructure.

Open the next Gruntbook: 

```bash
gruntbooks open testdata/sample-gruntbooks/demo2/gruntbook.mdx
```