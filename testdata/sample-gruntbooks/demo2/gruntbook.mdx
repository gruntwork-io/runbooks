...
In this Gruntbook, we'll create the `infrastructure-live-root` repo.
...
<Command id="create-infrastructure-live-root-repo"
    command="gh repo create {{ .inputs.GithubOrgName }}/{{ .inputs.GithubRepoName }} --private"
...
<Check id="check-infrastructure-live-root-repo"
    command="gh repo view {{ .inputs.GithubOrgName }}/{{ .inputs.GithubRepoName }}"
...
<TemplateInline id="ci-workflow-preview" inputsId="infrastructure-live-root-repo-inputs" outputPath="ci.yml" generateFile={false}>
```yaml
name: CI
...
Now that you've created the `infrastructure-live-root` repo, let's move on to the next gruntbook where we'll create the `infrastructure-live-access-control` repo.

Open the next Gruntbook: 

```bash
gruntbooks open testdata/sample-gruntbooks/demo3/gruntbook.mdx
```