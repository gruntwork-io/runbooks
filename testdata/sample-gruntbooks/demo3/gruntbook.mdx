...
This gruntbook tests various code block extraction scenarios to ensure MDX code blocks work correctly with the custom CodeBlock component.
...
variables:
  ...
  - name: GithubRepoName
    ...
    default: "gruntbooks-infrastructure-live-example"
    validations:
      - required
```
</Inputs>
...
<TemplateInline id="test3-template" inputsId="test2" outputPath="test3-output.yml">
```yaml
# Test 3 Output
orgName: {{ .inputs.GithubOrgName }}
repoName: {{ .inputs.GithubRepoName }}
```
</TemplateInline>
...
<TemplateInline id="test4-template" inputsId={["test1", "test2"]} outputPath="test4-output.yml">
```yaml
# Test 4 Output - Combined Variables
orgName: {{ .inputs.GithubOrgName }}
orgNamePrefix: {{ .inputs.OrgNamePrefix }}
```
</TemplateInline>
...
<Command id="test6-command"
    command="echo 'Project: {{ .inputs.ProjectName }} by {{ .inputs.AuthorName }}'"
...
<TemplateInline id="test7-template" inputsId="test6-inputs" outputPath="test7-output.txt">
```text
