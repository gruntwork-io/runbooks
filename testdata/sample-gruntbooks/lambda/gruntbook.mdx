...
This gruntbook guides you through creating a new AWS Lambda function using Terragrunt and the [Lambda module](https://github.com/gruntwork-io/terragrunt-infrastructure-catalog-example/tree/main/modules/lambda-service) from the Terragrunt Infrastructure Catalog Example.
...
First, authenticate to AWS. This gruntbook uses AWS SSO (IAM Identity Center) for authentication.

<AwsAuth
  id="aws-auth"
  title="Authenticate to AWS"
  description="Authenticate to the AWS account where you want to deploy the Lambda function."
