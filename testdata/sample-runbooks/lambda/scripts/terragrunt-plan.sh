...
# Run terragrunt plan for the Lambda function
...
# Run terragrunt plan
echo "📋 Running terragrunt plan for: $FUNCTION_NAME-$ENVIRONMENT in $AWS_REGION..."
echo "   Path: ${TARGET_PATH}"
echo ""
...
if terragrunt run --backend-bootstrap --non-interactive -- plan; then
  ...
  echo "   If the plan looks good, run terragrunt apply to deploy the changes."
  exit 0
else
  echo ""
