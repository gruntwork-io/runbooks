...
- **On success:** outcome `valid`; auth is accepted and session env written as usual; the success card gains a transparency line — *"validated via glab CLI — Runbooks' direct connection to `<H>` is not trusted yet"* — linking the TLS remediation card (§7). A structured log line `transport degraded for <host>: <code>` doubles as a support signal and a field canary for Node system-store-reader regressions. No degraded-host state is kept: later direct API calls for the host (labels, MR create) are not short-circuited (see §12). git operations are unaffected.
...
5. **Reload** re-runs enumeration, invalidates the CLI token cache, and re-runs `installSystemTrust` **with the cold-read refresh** — the documented path after `glab auth login --hostname new.host` in a terminal *or* after installing a CA mid-session.
...
- **§6 interface, extended:** `VcsCredentials` additionally exposes per-source detection legs (`detectGitHubEnv/Cli`, `detectGitLabEnv/Cli`) and `validateDirect` — the renderer's per-source `detectCredentials` contract is preserved unchanged (§2), so the per-source channels need per-leg service methods. The source chain itself (first valid wins, `invalid` continues, `unreachable` stops) runs in the renderer (`useGitAuth.ts`) over those channels, in the author's `detectCredentials` order, so the service has no `resolveGitHub/resolveGitLab`.
- **§2.4 transport-degraded state dropped:** the labels/MR-create short-circuit was never wired, so the degraded-host set had no reader. `markTransportDegraded` only emits the `transport degraded for <host>: <code>` log line; there is no `isTransportDegraded`/`clearTransportDegraded`, and `vcs:invalidate-cache` only flushes the caches.
