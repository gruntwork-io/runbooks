...
/**
 * GruntbookContext - Shared State Management for Gruntbook Blocks
 *
 * This context enables communication between different block components in a gruntbook.
 ...
 * execution. Scripts can write to $GRUNTBOOK_OUTPUT to expose values (e.g., account IDs,
 ...
 * 3. Script writes `account_id=123` to $GRUNTBOOK_OUTPUT
 ...
 *
 * ```tsx
 * const { registerInputs, getInputs } = useGruntbookContext()
 *
 ...
 * Outputs are always strings (written via $GRUNTBOOK_OUTPUT as key=value pairs).
 */
 ...
 */
export interface GruntbookContextType {
  /** The gruntbook name derived from its directory path (e.g., "github-pull-request") */
  gruntbookName: string | undefined

  /** The original remote URL when the gruntbook was opened from a remote source (e.g., GitHub URL).
   * Used by TfModule's source="::cli_gruntbook_source" keyword to resolve the module source dynamically. */
...
export const GruntbookContext = createContext<GruntbookContextType | undefined>(undefined)
 ...
 * </GruntbookContextProvider>
 */
export function GruntbookContextProvider({ children, gruntbookName, remoteSource }: { children: ReactNode, gruntbookName?: string, remoteSource?: string }) {
  ...
  const registerInputs = useCallback((id: string, values: Record<string, unknown>, config: BoilerplateConfig) => {
    setBlockInputs(prev => {
      ...
      console.log(`[GruntbookContext] registerInputs updating [${id}]: keys=${Object.keys(values).length}, configVars=${config?.variables?.length ?? 0}`)
      return {
        ...prev,
        [id]: { values, config }
      }
  ...
  const registerOutputs = useCallback((blockId: string, values: Record<string, string>) => {
    ...
    console.log(`[GruntbookContext] registerOutputs [${blockId} → ${normalizedId}]: keys=${Object.keys(values).length}`)
  ...
  const contextValue = useMemo(() => ({
    ...
    getOutputs,
    getTemplateContext,
  }), [gruntbookName, remoteSource, blockInputs, registerInputs, getInputs, blockOutputs, registerOutputs, getOutputs, getTemplateContext])

  return (
    <GruntbookContext.Provider value={contextValue}>
      {children}
    </GruntbookContext.Provider>
  )
}

// Hooks are in a separate file to satisfy react-refresh requirements
// See: useGruntbook.ts for useGruntbookContext and other hooks
