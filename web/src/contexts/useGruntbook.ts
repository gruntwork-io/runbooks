 ...
 *
 * ### Block Outputs
 * When a Check or Command block executes a script that writes to $GRUNTBOOK_OUTPUT,
 ...
 * const { registerOutputs } = useGruntbookContext()
 * registerOutputs(blockId, [{ name: "account_id", value: "123456789012" }])
 * ```
 *
 * @see GruntbookContext - The context provider
 */

import { useContext, useMemo, useRef } from 'react'
import { GruntbookContext, type GruntbookContextType, type TemplateValue, type OutputValue, type BlockOutputs } from './GruntbookContext'
import type { TemplateContext } from '@/lib/templateUtils'

// Re-export types and helpers for convenience
export { type TemplateValue, type OutputValue, type BlockInputs, type BlockOutputs, flattenInputs, valuesToOutputs } from './GruntbookContext'
...
/**
 * Hook to access the full gruntbook context.
 * Use this when you need to register inputs or outputs.
 */
export function useGruntbookContext(): GruntbookContextType {
  const context = useContext(GruntbookContext)
  if (!context) {
    throw new Error('useGruntbookContext must be used within a GruntbookContextProvider')
  }
  return context
}
